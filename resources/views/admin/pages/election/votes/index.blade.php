...
@section('content')
    <div class="container my-5">
        ...
        <form class="d-flex" method="GET" action="{{ route('votes.index') }}" role="search">
            ...
            <a href="{{ route('votes.create') }}" class="button-index ms-2">
        ...
        <table class="table mt-2 table-light table-hover">
            ...
            <tbody>
                @foreach ($votes as $vote)
                    <tr>
                        <td>{{ $vote->precincts->number }}</td>
                        <td>{{ $vote->candidates->fullname }}</td>
                        <td>{{ $vote->actual_votes }}</td>
                        <td>
                            {{-- <a href="{{ route('votes.show', $vote->id) }}" class="icon-link" title="Show">
                                <i class="fas fa-eye"></i>
                            </a> --}}
                            <a href="{{ route('votes.edit', $vote->id) }}" class="icon-link" title="Edit">
