...
@section('content')
    <div class="container my-5">
        ...
        <form class="d-flex mb-3 justify-content-between" method="GET" action="{{ route('voters.summary') }}" role="search">
            ...
            <tbody>
                @foreach($data as $item)
                    <tr>
                        ...
                        <td>{{ $item['downLine'] }}</td>
                        <td>{{ $item['totalLeadersAndDownline'] }} out of {{ $item['total'] }} ({{ number_format($item['percentage'], 1) }}%)</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
